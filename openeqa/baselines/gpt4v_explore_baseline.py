...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dataset",
        type=Path,
        default="data/open-eqa-subset-questions.json",
    ...
    args.output_path = args.output_directory / (
        args.dataset.stem + "-baseline-{}-{}.json".format(args.model, args.seed)
    )
    return args
...
def ask_question(
    ...
    try:
        ...
        output = call_openai_api(
            ...
            api_key="0186e6b94e834c4db7d3a2f7e5303fff",
        )
        return output
...
def main(args: argparse.Namespace):
    ...
    for idx, item in enumerate(tqdm.tqdm(dataset)):
        ...
        # extract scene paths
        folder = args.frames_directory / question_id / "observations"
        # frames = os.listdir(folder)
        # frames = [frame for frame in frames if frame.endswith(".png")]
        # frames = sorted(frames)
        frames = sorted(folder.glob("*.png"))
        assert len(frames) >= 50, "expected at least 50 frames"
